...
function resolveNodePath() {
  // Nếu có bundled Node.js runtime, dùng luôn
  if (fs.existsSync(NODE_FALLBACK)) return NODE_FALLBACK

  // Quét PATH + common macOS Homebrew paths
  // macOS Finder launch chỉ có PATH=/usr/bin:/bin → fallback extraPaths
  const searchDirs = [
...
function startProdBackend() {
  ...
  if (!nodePath || !fs.existsSync(backendPath)) {
    console.log('[Electron] Backend not found, skipping:', { node: nodePath, backend: backendPath })
    return
  }
...
async function createOverlay() {
  ...
  const storedHubWidth = Number(readConfig().hubWidth) || 440
...
ipcMain.on('overlay:resize-width', (_, newWidth) => {
  ...
  const clamped = Math.min(720, Math.max(260, Math.round(Number(newWidth)) || 440))
